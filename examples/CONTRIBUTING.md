...
3. **Add the experiment report and any small supporting files** you reference.
   Make the report with
   `python scripts/experiment_report.py experiments/<experiment-id> --no-pdf` and add
   the `report.html` that it writes (one file, with all the images inside). Also add
   `config_used.json`, small result CSVs and figures. Keep it small (target under 50 MB): commit the recipe and results, not
