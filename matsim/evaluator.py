...
class SimulationEvaluator:
    ...
    def run_evaluation(self, network_path: Optional[Path] = None,
        ...
        """
        # Trip timing and activity duration, generated demand against the last
        # iteration. Drawn first because it needs no counts: a run without
        # matched devices or countscompare.txt still gets these. A failure here
        # must not cost the counts evaluation, so it is logged and skipped.
        try:
            from matsim.trip_timing import plot_trip_timing
            plot_trip_timing(self.experiment_dir, self.evaluation_dir)
        except Exception as e:
            logger.warning(f"Trip timing figures failed: {e}", exc_info=True)
